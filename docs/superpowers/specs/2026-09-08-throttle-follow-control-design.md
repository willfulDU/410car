...
为 STM32F103C8 智能车增加独立、可复用的整车控制层。该控制层支持两种单踏板油门方式和基于 PID 的定距跟随状态机，同时保留已有的底层驱动，并在车辆前进时执行 150 mm 的自动紧急制动限制。三块 ECU 按“方向盘采集、测距计算、主 ECU 最终执行”分工，避免多个控制器同时写电机输出。
...
系统包含方向盘 ECU、车端主 ECU 和车端测距 ECU。三块 ECU 都使用 STM32F103C8，但控制权不对等：方向盘 ECU 产生驾驶意图，测距 ECU 产生测距结果和跟随建议，主 ECU 是唯一拥有电机最终控制权的节点。

| ECU | 负责内容 | 建议周期 | 不负责内容 |
| --- | --- | ---: | --- |
| 方向盘 ECU | 方向盘 ADC、单踏板 ADC、D/R/N 开关、左右转向开关；输入去抖、限幅、NRF24L01 控制帧发送 | 10 ms | 不运行车辆 PID，不直接控制电机 |
| 车端主 ECU | NRF24L01 接收、测距串口接收、FG 轮速采样、速度 PID、电机 PWM、转向舵机、WS2812、OLED、AEB 和故障仲裁 | 10 ms 控制周期 | 不执行 VL53L0X 底层测量 |
| 车端测距 ECU | VL53L0X 测量、距离有效性判断、距离滤波、跟随距离 PID、测距 UART 帧发送 | 40 ms，约 25 Hz | 不输出电机 PWM，不绕过主 ECU 安全判断 |

### 控制链路

```text
方向盘 ECU
  └─ NRF24L01：踏板、档位、方向盘、转向灯
                 ↓
车端主 ECU ──────┐
  ├─ 速度 PID     │
  ├─ AEB 150 mm   │
...
车端测距 ECU ────┘
  ├─ VL53L0X
  ├─ 距离滤波
  └─ 距离 PID：250 mm → 建议目标 RPM
```

测距 ECU 只能向主 ECU 提供“距离”和“建议目标转速”。主 ECU 必须再次检查无线状态、档位、距离新鲜度、轮速反馈和 AEB 条件，之后才允许调用 `Motor_SetCW()`、`Motor1_SetDuty()` 和 `Motor2_SetDuty()`。
...
4. `protocol/`：维护三块 ECU 共用的消息格式、序号、状态位和 CRC。方向盘无线帧继续保持现有 8 字节格式；测距 ECU 到主 ECU 使用独立的固定长度 UART 二进制帧。
5. `ranging/`：放置测距 ECU 的距离滤波、有效性检查、跟随 PID 和 UART 发送，避免测距算法侵入主 ECU。
...
测距 ECU 不再用 `printf("%d\\n")` 作为控制数据。第一版采用固定 9 字节 UART 帧：

```text
0xA5 | 0x5A | 序号 |
距离高字节 | 距离低字节 |
建议目标 RPM 高字节 | 建议目标 RPM 低字节 |
状态位 | CRC8
```

状态位至少包含距离有效、跟随建议有效、AEB 警告和传感器故障。主 ECU 使用串口中断与环形缓冲区接收，不在 10 ms 控制周期中阻塞等待字符。若在配置的超时时间内未收到新帧，主 ECU 将距离标记为无效。

## 主 ECU 的安全优先级

主 ECU 按以下优先级决定最终电机命令：

1. 无线遥控超时或控制帧无效：停车。
2. 前进且有效距离小于等于 150 mm：自动紧急制动，前进 PWM 为零。
3. 跟随模式中测距帧超时或距离无效：禁止前进。
4. N 档或零踏板：停车。
5. 跟随模式：采用测距 ECU 的建议目标 RPM，经主 ECU 速度 PID 后输出。
6. 手动模式：采用方向盘 ECU 的踏板目标，按模式 1 或模式 2 输出。

倒车命令不被 AEB 阻断，手动模式下始终允许通过倒车离开障碍物。跟随模式本身只允许前进，收到 R 档时输出停车，等待后续由用户明确关闭跟随。

## 控制任务调度

主 ECU 使用 1 ms 时间基准和非阻塞任务，不让无线重试、OLED 刷屏或串口接收破坏速度控制周期：

| 任务 | 周期 |
| --- | ---: |
| 无线、UART 帧解析 | 持续运行，非阻塞 |
| AEB、速度 PID、电机输出 | 10 ms |
| FG 脉冲统计与轮速滤波 | 20 ms |
| 舵机、灯光逻辑 | 20 ms |
| OLED 刷新 | 100 ms |
| 通信超时检测 | 10 ms |

测距 ECU 的 VL53L0X 单次测量预算约 33 ms，因此采用 40 ms 测距周期；主 ECU 只消费最新有效帧，不等待测量完成。
...
- `RANGE_UART_BAUDRATE = 115200`
- `RANGE_UART_TIMEOUT_MS`：测距帧失效超时，由实现阶段根据 25 Hz 发送周期设置。
...
3. 从 UART 环形缓冲区解析最新测距帧；没有新帧或 CRC 错误时保留“距离无效”。
4. 从反馈模块取得轮速。第一版将其标记为无效。
5. 调用 `VehicleControl_Update()`，传入当前控制周期和全部有效性标志。
...
- UART 固定帧的帧头、长度、序号和 CRC 校验。
- 测距帧超时后禁止主 ECU 前进，恢复有效帧后恢复可控状态。
- 测距 ECU 的建议目标 RPM 不能直接改变电机输出，必须经过主 ECU 安全仲裁。
- 方向盘 ECU 发送周期异常或无线接收超时时主 ECU 停车。
